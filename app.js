...
const questions = [
    ...
    { text: "Недельная норма проверок состава МО от сотрудников ВП?" }
];
...
function loadTestState() {
    ...
    if (saved) {
        ...
        // Если тест был заблокирован, блокируем интерфейс
        if (blocked) {
...
function blockTest() {
    ...
    // Создаем файл с кодом разблокировки (зашифрованный)
...
function renderTest() {
    ...
    if (test.blocked) {
        area.innerHTML = `
            <div class="blocked-note">
                <h2>🚫 Тест заблокирован!</h2>
                <p>Отправьте файл с кодом разблокировки администратору.</p>
                <p>Введите код разблокировки от администратора:</p>
                <input type="text" id="unlockInput" placeholder="Введите код от администратора" style="margin: 10px 0; width: 100%;">
                <button class="btn" id="unlockWithCodeBtn">Разблокировать тест</button>
            </div>
        `;
        
        document.getElementById("unlockWithCodeBtn").addEventListener("click", () => {
    ...
    document.getElementById("answerInput").addEventListener("input", (e) => {
        ...
        saveTestState(); // Сохраняем после каждого ответа
    });
